'use client'; // Wajib ditambahkan jika menggunakan event onClick di App Router Next.js

// 1. DATA PRODUK: Edit, tambah, atau hapus produk dari sini
const products = [
  { 
    id: 1, 
    name: "Vas Keramik Bali", 
    price: "Rp 150.000", 
    image: "https://images.unsplash.com/photo-1612196808214-b8e1d6145a8c?q=80&w=600&auto=format&fit=crop" 
  },
  { 
    id: 2, 
    name: "Lampu Hias Rotan", 
    price: "Rp 250.000", 
    image: "https://images.unsplash.com/photo-1513506003901-1e6a229e2d15?q=80&w=600&auto=format&fit=crop" 
  },
  { 
    id: 3, 
    name: "Karpet Anyaman", 
    price: "Rp 300.000", 
    image: "https://images.unsplash.com/photo-1558997519-83ea9252edf8?q=80&w=600&auto=format&fit=crop" 
  },
];

export default function Home() {
  
  // 2. FUNGSI WHATSAPP: Mengirim pesan dinamis
  const handleOrder = (productName) => {
    const phoneNumber = "6281234567890"; // Ganti dengan nomor WA Anda (awali dengan 62 tanpa + atau 0)
    const message = `Halo admin Unikubali, saya tertarik untuk membeli produk *${productName}*. Apakah stoknya masih tersedia?`;
    
    // Mengubah teks menjadi format URL yang valid (mengganti spasi dengan %20, dst)
    const encodedMessage = encodeURIComponent(message);
    const whatsappUrl = `https://wa.me/${phoneNumber}?text=${encodedMessage}`;
    
    // Membuka tab baru ke WhatsApp
    window.open(whatsappUrl, '_blank');
  };

  // 3. TAMPILAN WEBSITE (UI)
  return (
    <main className="min-h-screen p-8 font-sans bg-gray-50">
      <header className="mb-12 text-center pt-10">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-800 tracking-wide">UNIKUBALI</h1>
        <p className="text-gray-500 mt-3 text-lg">Koleksi Aksesoris Rumah Estetik</p>
      </header>

      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 max-w-6xl mx-auto">
        {products.map((product) => (
          <div key={product.id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl transition-all duration-300 group">
            <div className="overflow-hidden h-64">
              <img 
                src={product.image} 
                alt={product.name} 
                className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" 
              />
            </div>
            <div className="p-6">
              <h2 className="text-xl font-semibold text-gray-800">{product.name}</h2>
              <p className="text-green-600 font-bold mt-2 text-lg">{product.price}</p>
              
              <button 
                onClick={() => handleOrder(product.name)}
                className="mt-5 w-full bg-green-600 text-white font-medium py-3 rounded-lg hover:bg-green-700 active:bg-green-800 transition-colors flex items-center justify-center gap-2"
              >
                {/* Ikon sederhana menggunakan SVG */}
                <svg className="w-5 h-5" fill="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347m-5.421 7.403h-.004a9.87 9.87 0 01-5.031-1.378l-.361-.214-3.741.982.998-3.648-.235-.374a9.86 9.86 0 01-1.51-5.26c.001-5.45 4.436-9.884 9.888-9.884 2.64 0 5.122 1.03 6.988 2.898a9.825 9.825 0 012.893 6.994c-.003 5.45-4.437 9.884-9.885 9.884m8.413-18.297A11.815 11.815 0 0012.05 0C5.495 0 .16 5.335.157 11.892c0 2.096.547 4.142 1.588 5.945L.057 24l6.305-1.654a11.882 11.882 0 005.683 1.448h.005c6.554 0 11.89-5.335 11.893-11.893a11.821 11.821 0 00-3.48-8.413z"/></svg>
                Pesan via WhatsApp
              </button>
            </div>
          </div>
        ))}
      </div>
    </main>
  );
}